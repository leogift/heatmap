...
from heatmap.data import DataPrefetcher
from heatmap.exp import Exp
...
class Trainer:
    ...
    def before_train(self):
        ...
        logger.info(
            "Model Summary: {}".format(get_model_info(origin_model,
                                                    len(self.exp.category_list),
                                                    self.exp.max_points_per_category,
                                                    self.exp.image_size
                                                ))
            )
    ...
    def before_epoch(self):
        from heatmap.utils import freeze_module, unfreeze_module
        # trainning mode
        self.model.train()
