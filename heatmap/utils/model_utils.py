...
def get_model_info(model: nn.Module, \
    ...
    model.train()
    flops, params = profile(deepcopy(model), inputs=(image,
        mask, 
        kpts_xy,kpts_visible), verbose=False)
...
def optimize_model(model: nn.Module) -> nn.Module:
    ...
    """
    from heatmap.models.network_blocks import BaseConv, RepSConv, RepBottleneck
